...
class FCN(nn.Module):
    ...
    def forward(self,X_train):
        ...
        if self.edge_net:
            ...
            # X_c = X_c.reshape(X_c.shape[0],-1,h,w) #b,c,h,w
...
class MDL(nn.Module):
    def __init__(self,node_conf,node_tconf,node_pconf,
                 edge_conf,edge_tconf,edge_pconf,external_dim=28,embed_dim=16,bridge='concat'):
        super(MDL, self).__init__()

        t, c, h, w = edge_conf

        self.node_net = FCN(c_conf=node_conf, p_conf=node_tconf, t_conf=node_pconf, nb_residual_unit=2, embed_dim=embed_dim, edge_net=False)
        self.edge_net = FCN(c_conf=edge_conf, p_conf=edge_tconf, t_conf=edge_pconf, nb_residual_unit=2, embed_dim=embed_dim, edge_net=True)
    ...
    def multask_loss(self,X,M,X_ext,X_gt,M_gt):
        ...
        X_gt = torch.reshape(X_gt, (b,-1,h,w))
        M_gt = torch.reshape(M_gt, (b,-1,h,w))
...
if __name__ == '__main__':
    ...
    model = MDL(node_conf=(3,2,8,8),node_tconf=(1,2,8,8),node_pconf=(1,2,8,8),
                 edge_conf=(3,512,8,8),edge_tconf=(1,512,8,8),edge_pconf=(1,512,8,8))
    ...
    # M_c = torch.rand((64,3*512,8,8))
    #
    # conv1 = nn.Conv2d(in_channels=3*512,out_channels=6,kernel_size=(1,1),stride=1,padding=0)
    #
    # out = conv1(M_c)
    #
    # print(out.shape)